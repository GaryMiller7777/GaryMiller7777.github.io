...
var AITextWindow = function(id, parent, init_data, text_type) {
			...
			$('<div class="scroller-container' + (supports3d ? ' supports-3d' : '') + '">'+
				...
				'<div class="scroller-flipper">' +
					'<div class="scroller-main">' +
						'<div class="scroller-text-wrapper reading-text"></div>' +
					'</div>'+
	...
	*/
	...
	function init() {
		...
		console.log('ai-textsindow init', init_data, isInitialized, text_type);
		...
		/* For initial prototyping, no text loading in ai-textwindow
	...
	wrapper.html('Bible Engagement with AI Assistant');

	//import { tryai } from '../lib/tryai.js'
	//tryai = require('../lib/tryai.js')
	//tryai();
...
var ChatGPTWindow = function(id, node, init_data) {
	return new AITextWindow(id, node, init_data, 'chatgpt');
};

sofia.initMethods.push(function() {

	if (typeof sofia.config.enableAIQuery != 'undefined') {

		sofia.windowTypes.push({
			className:'ChatGPTWindow',
			param: 'chatgpt',
			paramKeys: {
				'textid': 't',
				'fragmentid':'v'
